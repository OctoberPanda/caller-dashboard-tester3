...
let cfg={},banks=[],logs={},flags={},calls={},apptHeld={},emailLogs={},callMeta={},blackbox={},hqFlags={},hqMeta={},openRI=null,numCtx=null,genCtx=null,undoCtx=null,workDate='';
...
window.onload=()=>{
  cfg=loadCfg();logs=loadLogs();flags=loadFlags();calls=loadCalls();apptHeld=loadAppt();emailLogs=loadEmailLogs();callMeta=loadCallMeta();blackbox=loadBlackbox();hqFlags=loadHQFlags();hqMeta=loadHQMeta();
...
function openNumModal(ri,role){
  ...
  // ── Per-number controls: add extension/option + black-box toggle ──
...
function showEOD(){
  ...
  const buildText=(dials)=>{
    ...
    // 2x and 7x numbers, grouped separately
    const rule2x=[],rule7x=[];
    Object.values(flags).filter(f=>!f.undone&&!f.fromSheet&&f.date===workDate).forEach(f=>{
      const b=banks.find(x=>x.ri===f.ri);if(!b)return;
      const line='Row '+f.ri+' | '+b.d[C.BANK]+' | '+f.role+' | '+phoneBase(f.phone);
      if(f.rule==='2x')rule2x.push(line);
      else if(f.rule==='7x')rule7x.push(line);
    });
    if(rule2x.length){t+='\n— 2x Numbers (black box, no answer) —\n\n';rule2x.forEach(l=>t+=l+'\n');}
    if(rule7x.length){t+='\n— 7x Numbers (reached bank/lead, no answer) —\n\n';rule7x.forEach(l=>t+=l+'\n');}

    // HQ flagged numbers (app only)
    const hqList=Object.values(hqFlags).filter(f=>!f.undone&&f.date===workDate).map(f=>{
      const b=banks.find(x=>x.ri===f.ri);
      return 'Row '+f.ri+' | '+(b?b.d[C.BANK]:'')+' | HQ '+f.phone+' | '+f.issue;
    });
    if(hqList.length){t+='\n— Flagged HQ Numbers (app only) —\n\n';hqList.forEach(l=>t+=l+'\n');}
...
function buildHQRow(ri,hqRaw){
  ...
  h+='<span class="bank-hq'+(flagged?' hq-flagged':'')+'" onclick="copyPhone(\''+esc(base)+'\',this)" title="Click to copy">🏢 HQ: '+esc(hqRaw)+(flagged?' ⚑':'')+cntLabel+'</span>';
  h+='<button class="hq-btn" onclick="hqDialNoAnswer('+ri+')" title="Log a no-answer dial">📵 No answer</button>';
  h+='<button class="hq-btn'+(bb?' hq-btn-on':'')+'" onclick="hqToggleBlackbox('+ri+')" title="Black box = 2x rule">📦 '+(bb?'Black box ✓':'Black box')+'</button>';
  if(flagged){h+='<button class="hq-btn hq-btn-undo" onclick="hqUndoFlag('+ri+')">↩ Unflag</button>';}
  else{h+='<button class="hq-btn hq-btn-flag" onclick="hqFlagPick('+ri+')">⚑ Flag</button>';}
  h+='</div>';
  return h;
}

function hqDialNoAnswer(ri){
  const k=hqKey(ri);
  if(!hqMeta[k])hqMeta[k]={na:0,at:0,blackbox:false};
  const bb=hqMeta[k].blackbox;
  ...
  if(bb&&hqMeta[k].na>=2&&!isHQFlagged(ri)){
    hqFlags[k]={ri,phone:phoneBase(String(b.d[C.HQ])),issue:'2x no answer (black box)',undone:false,date:workDate,rule:'2x'};saveHQFlags();
    toast('HQ auto-flagged: 2x','success');
  }else if(!bb&&hqMeta[k].at>=7&&!isHQFlagged(ri)){
    hqFlags[k]={ri,phone:phoneBase(String(b.d[C.HQ])),issue:'7x no answer (reached bank)',undone:false,date:workDate,rule:'7x'};saveHQFlags();
    toast('HQ auto-flagged: 7x','success');
  }else{
    toast('HQ no-answer logged','success');
  }
  showCurrentBank();
}

function hqToggleBlackbox(ri){
  const k=hqKey(ri);
  if(!hqMeta[k])hqMeta[k]={na:0,at:0,blackbox:false};
  hqMeta[k].blackbox=!hqMeta[k].blackbox;saveHQMeta();
  toast(hqMeta[k].blackbox?'HQ marked black box (2x rule)':'HQ unmarked','success');
  showCurrentBank();
}

function hqFlagPick(ri){
  const b=banks.find(x=>x.ri===ri);if(!b)return;
  const opts=FLAG_OPTIONS.map((f,i)=>(i+1)+'. '+f).join('\n');
  const pick=prompt('Flag HQ number '+phoneBase(String(b.d[C.HQ]))+' (app only). Enter number:\n\n'+opts);
  if(pick===null)return;
  const idx=parseInt(pick)-1;
  if(idx<0||idx>=FLAG_OPTIONS.length){toast('Invalid choice','error');return;}
  hqFlags[hqKey(ri)]={ri,phone:phoneBase(String(b.d[C.HQ])),issue:FLAG_OPTIONS[idx],undone:false,date:workDate};
  saveHQFlags();showCurrentBank();toast('HQ flagged (app only)','success');
}
