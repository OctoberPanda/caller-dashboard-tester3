...
let cfg={},banks=[],logs={},flags={},calls={},apptHeld={},emailLogs={},callMeta={},blackbox={},hqFlags={},hqMeta={},dne={},appFU={},parked={},fud={},fuByBank={},openRI=null,numCtx=null,genCtx=null,undoCtx=null,workDate='';
...
setInterval(()=>{try{if(banks.length)applyFilters(false);}catch(e){}},15*60*1000);

window.onload=()=>{
  cfg=loadCfg();logs=loadLogs();flags=loadFlags();calls=loadCalls();apptHeld=loadAppt();emailLogs=loadEmailLogs();callMeta=loadCallMeta();blackbox=loadBlackbox();hqFlags=loadHQFlags();hqMeta=loadHQMeta();dne=loadDNE();appFU=loadAFU();parked=loadParked();fud=loadFUD();
...
function saveSetup(){
  const name=gv('s-name').trim(),sheetId=gv('s-sheet-id').trim(),tab=gv('s-tab').trim(),updateSheetId=gv('s-update-id').trim(),updateTab=gv('s-update-tab').trim(),followupTab=gv('s-followup-tab').trim(),apiKey=gv('s-api-key').trim();
  if(!name||!sheetId||!tab||!apiKey){toast('Please fill in all required fields','error');return;}
  cfg={name,sheetId,tab,updateSheetId,updateTab,followupTab,apiKey,lastWorkDate:workDate};saveCfg();show('main-app');boot();
}
...
function renderList(list){
  // Remember which bank the user is currently on
  const currentRi=navList&&navList[navIdx]?navList[navIdx].ri:null;
  // Follow-up banks first, then due app-followups, then the rest
  navList=list.slice().sort((a,b)=>{
    const pa=fuActive(a.ri)?0:(bankHasDueAFU(a.ri)?1:2);
    const pb=fuActive(b.ri)?0:(bankHasDueAFU(b.ri)?1:2);
    return pa-pb;
  });
  list=navList;
  // Pin the user to the same bank after any reorder
  if(currentRi!==null){
    const keep=navList.findIndex(x=>x.ri===currentRi);
    if(keep>=0)navIdx=keep;
  }
  // Clamp index
  ...
  div.innerHTML=`
    <div class="bank-header">
      <div style="display:flex;align-items:flex-start;justify-content:space-between">
        ...
        <div class="bank-badges">${badges}<button class="hq-btn" onclick="toggleParked(${ri})">${isParked(ri)?'↩ Back to main list':'📂 Move off main list'}</button></div>
      </div>
    </div>
...
function buildLeadCard(ri,d,role,bankDeclined){
  ...
  return '<div class="lead-card'+(hasSOS?' sos':'')+(hasInt?' interest':'')+(called?' complete-lead':'')+(bankDeclined?' declined-lead':'')+(leadDNE?' declined-lead':'')+'"><div class="lead-header"><div class="lead-header-left"><div class="lead-role-row"><span class="role-tag">'+role+'</span>'+statusTag+(outcome?'<span class="outcome-chip '+oc+'">'+esc(outcome)+'</span>':'')+'</div><div class="lead-name">'+esc(name)+'</div>'+(ea?'<div class="lead-ea">EA: '+esc(ea)+'</div>':'')+'</div><div class="lead-header-right">'+(recent?'Last: '+recent+'<br>':'')+times+'x total</div></div><div class="lead-body">'+afuHtml+attn+phonesHtml+notesHtml+todayHtml+bottomAction+'</div></div>';
}
  ...
  const buildText=(dials)=>{
    ...
    if(gkReached.length){
      gkReached.slice().sort((a,b)=>a.row-b.row).forEach(c=>{
        const note=c.note.replace(/\.+$/,'').trim();
        t+='Row '+c.row+' | '+cleanBank(c.bank)+' | '+note+'\n';
      });
    }else{t+='None\n';}
    ...
    // Flagged HQ numbers
    const hqList=Object.values(hqFlags).filter(f=>!f.undone&&f.date===workDate)
      .sort((a,b)=>a.ri-b.ri).map(f=>{
        const b=banks.find(x=>x.ri===f.ri);
        return 'Row '+f.ri+' | '+cleanBank(b?b.d[C.BANK]:'')+' | HQ '+f.phone+' | '+cleanIssue(f.issue);
      });
    if(hqList.length){t+='\nFlagged HQ Numbers\n\n';hqList.forEach(l=>t+=l+'\n');}
    ...
    // Flagged Numbers Report: ONE line per bank+number, shared numbers merged (CRA & CEO), sorted by row
    ...
    if(todayFlags.length){
      ...
      // Group by bank row, then by DIGITS (so (815) 847-7500 and 815.847.7500 merge)
      const byBank={};
      todayFlags.forEach(f=>{
        if(!byBank[f.ri])byBank[f.ri]={};
        const digits=phoneDigits(f.phone)||phoneBase(f.phone);
        if(!byBank[f.ri][digits])byBank[f.ri][digits]={roles:[],issue:cleanIssue(f.issue),display:''};
        if(!byBank[f.ri][digits].roles.includes(f.role))byBank[f.ri][digits].roles.push(f.role);
        // Always display in clean XXX.XXX.XXXX format when we have 10 digits
        const dg=phoneDigits(f.phone);
        byBank[f.ri][digits].display=dg.length===10?dg.slice(0,3)+'.'+dg.slice(3,6)+'.'+dg.slice(6):phoneBase(f.phone);
      });
...
function showSettings(){sv('set-name',cfg.name||'');sv('set-sheet-id',cfg.sheetId||'');sv('set-tab',cfg.tab||'');sv('set-update-id',cfg.updateSheetId||'');sv('set-update-tab',cfg.updateTab||'');sv('set-followup-tab',cfg.followupTab||'');sv('set-api-key',cfg.apiKey||'');el('settings-modal').classList.remove('hidden');}
function closeSettings(){el('settings-modal').classList.add('hidden');}
function saveSettings(){
  cfg.name=gv('set-name').trim();cfg.sheetId=gv('set-sheet-id').trim();cfg.tab=gv('set-tab').trim();cfg.updateSheetId=gv('set-update-id').trim();cfg.updateTab=gv('set-update-tab').trim();cfg.followupTab=gv('set-followup-tab').trim();cfg.apiKey=gv('set-api-key').trim();
...
function fudGet(ri){const k=bankId(ri);if(!fud[k])fud[k]={dials:{},done:{},lastTouch:0,nextDate:''};return fud[k];}
...
function fudDial(ri){
  ...
  const dlog={id:genId(),ri,role:fuByBank[bankId(ri)]?.role||'CEO',who:'NO CONTACT',outcome:'No Answer',called:true,dialCount:1,date:workDate,fudDial:true,deleted:false};
...
function saveFUDDone(){
  ...
  const info=fuByBank[bankId(ri)];if(!info)return;
  const b=banks.find(z=>z.ri===ri);if(!b)return;
  const role=el('fud-role')?.value||info.role||'CEO';
  ...
  x.nextDate=nextDate;
  saveFUD();
  renderStats();closeNumModal();applyFilters(false);
