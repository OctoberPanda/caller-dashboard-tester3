...
function renderFollowups(){
  ...
  followups.forEach(f=>{
    ...
    if(mb){
      ...
      if(role&&RC[role]){const em=mb.d[RC[role].init];if(em)emailDate=fmtSheetDate(em);}
    }
...
function openFollowupModal(fri){
  ...
  let aa='',emailDate='';
  if(mb){aa=String(mb.d[C.AA]||'').trim();if(role&&RC[role]){const em=mb.d[RC[role].init];if(em)emailDate=fmtSheetDate(em);}}
...
function saveHQModal(){
  ...
  if(flagIssue){hqFlags[k]={ri,phone:base,issue:flagIssue,undone:false,date:workDate};}
