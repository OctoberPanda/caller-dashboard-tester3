...
function showEOD(){
  ...
  const buildText=(dials)=>{
    ...
    if(todayFlags.length){
      ...
      // Group by bank row, then by base number
      const byBank={};
      todayFlags.forEach(f=>{
        if(!byBank[f.ri])byBank[f.ri]={};
        const base=phoneBase(f.phone);
        if(!byBank[f.ri][base])byBank[f.ri][base]={roles:[],issue:cleanIssue(f.issue)};
        if(!byBank[f.ri][base].roles.includes(f.role))byBank[f.ri][base].roles.push(f.role);
      });
      Object.keys(byBank).map(Number).sort((a,b)=>a-b).forEach(ri=>{
        const b=banks.find(x=>x.ri===ri);
        const bankName=cleanBank(b?b.d[C.BANK]:'');
        Object.entries(byBank[ri]).forEach(([num,info])=>{
          t+='Row '+ri+' | '+bankName+' | '+info.roles.join(' & ')+' '+num+' | '+info.issue+'\n';
        });
      });
    }
    return t.trim();
