...
let cfg={},banks=[],logs={},flags={},calls={},apptHeld={},emailLogs={},callMeta={},blackbox={},openRI=null,numCtx=null,genCtx=null,undoCtx=null,workDate='';
...
window.onload=()=>{
  cfg=loadCfg();logs=loadLogs();flags=loadFlags();calls=loadCalls();apptHeld=loadAppt();emailLogs=loadEmailLogs();callMeta=loadCallMeta();blackbox=loadBlackbox();
    ...
    <div class="bank-header">
      <div style="display:flex;align-items:flex-start;justify-content:space-between">
        <div>
          ...
          ${d[C.HQ]?'<div class="bank-hq" onclick="copyPhone(\''+esc(phoneBase(String(d[C.HQ])))+'\',this)" title="Click to copy HQ number">🏢 HQ: '+esc(String(d[C.HQ]))+'</div>':''}
          ${emailRow}
        </div>
...
async function saveNumModal(){
  ...
  // Date updates every call. Times-called only goes up on a real connection:
  // someone other than NO CONTACT picked up AND it wasn't a no-answer/voicemail.
  const NON_CONNECT=['No Answer','Left Message'];
  const connected=(who!=='NO CONTACT')&&!NON_CONNECT.includes(outcome);
  d[rc.recent]=dateStr;
  if(connected)d[rc.times]=String((parseInt(d[rc.times])||0)+1);
...
async function saveGenLog(){
  ...
  d[rc.recent]=dateStr;const _conn=(who!=='NO CONTACT')&&!['No Answer','Left Message'].includes(outcome);if(_conn)d[rc.times]=String((parseInt(d[rc.times])||0)+1);d[rc.outcome]=outcome;d[rc.who]=who;
  if(newNum)d[rc.phone]=d[rc.phone]?d[rc.phone]+'; '+newNum:newNum;
