...
let cfg={},banks=[],logs={},flags={},calls={},apptHeld={},emailLogs={},callMeta={},blackbox={},hqFlags={},hqMeta={},dne={},appFU={},parked={},fud={},fuByBank={},leadFlags={},appNums={},openRI=null,numCtx=null,genCtx=null,undoCtx=null,workDate='';
...
window.onload=()=>{
  cfg=loadCfg();logs=loadLogs();flags=loadFlags();calls=loadCalls();apptHeld=loadAppt();emailLogs=loadEmailLogs();callMeta=loadCallMeta();blackbox=loadBlackbox();hqFlags=loadHQFlags();hqMeta=loadHQMeta();dne=loadDNE();appFU=loadAFU();parked=loadParked();fud=loadFUD();leadFlags=loadLeadFlags();appNums=loadAppNums();
...
function renderList(list){
  ...
  // Follow-up + priority banks first, then due app-followups, then the rest
  ...
  div.innerHTML=`
    <div class="bank-header">
      <div style="display:flex;align-items:flex-start;justify-content:space-between">
        ...
        <div class="bank-badges">${isPriorityBank(ri)?'<span class="badge" style="background:var(--amber-bg);color:var(--amber);border:1px solid var(--amber-border);font-weight:700">PRIORITY</span>':''}${badges}<button class="hq-btn" onclick="toggleParked(${ri})">${isParked(ri)?'↩ Back to main list':'📂 Move off main list'}</button></div>
      </div>
    </div>
...
function buildLeadCard(ri,d,role,bankDeclined){
  ...
  return '<div class="lead-card'+(hasSOS?' sos':'')+(hasInt?' interest':'')+(called?' complete-lead':'')+(bankDeclined?' declined-lead':'')+(leadDNE?' declined-lead':'')+'"><div class="lead-header"><div class="lead-header-left"><div class="lead-role-row"><span class="role-tag">'+role+'</span>'+statusTag+(outcome?'<span class="outcome-chip '+oc+'">'+esc(outcome)+'</span>':'')+'</div><div class="lead-name">'+esc(name)+'</div>'+(ea?'<div class="lead-ea">EA: '+esc(ea)+'</div>':'')+'</div><div class="lead-header-right">'+(recent?'Last: '+recent+'<br>':'')+times+'x total</div></div><div class="lead-body">'+afuHtml+leadFlagHtml+attn+phonesHtml+appNumHtml+notesHtml+todayHtml+bottomAction+emailBtn+'</div></div>';
}
